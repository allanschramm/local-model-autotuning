...
## Work Guidance
- **Runtime (do not rebuild)**: use the prebuilt release `llama.cpp-releases/upstream/b11149` (nightly, CUDA 13.4, post-`v0.5.0`) via `AUTORESEARCH_LLAMA_CPP_ROOT` — smoke-validated 2026-09-24 (`--version` 0.5.0-dev build 11149, CUDA backend loads); carries Nemotron-3.5 embedded-MTP (`nextn_predict_layers`) and `spark2_5` architecture support. `./llama.cpp` source is pinned at the `v0.5.0` submodule tag; its `build-cuda/` artifacts are stale and rebuilding is NOT the fix path (`b11149` supersedes `b10867`; `b10867`/`b10375` remain on disk — Machine-scope env still points at the old path, User scope is `b11149`).
