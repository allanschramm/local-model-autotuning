...
Read the latest row from the canonical results store (`results.db` via `autoresearch.core.results_db.load_rows` or direct SQLite query) and report the metrics:
...
| **Category** | `validation` | Logged to `results.db` (SQLite) | `incomplete` |
```
