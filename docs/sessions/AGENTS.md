...
- [`2026-09-24-llamacpp-v0.5.0-changelog.md`](./2026-09-24-llamacpp-v0.5.0-changelog.md) — analytical pass over upstream `v0.5.0` notes mapped to `ctx × TPS × agentic × coding`: ACTs are MTP-draft CUDA graphs re-bench, `fit-print` re-size, scheduler-reserve OOM signal, qwen3-coder think-budget re-check, `llama-bench --version` provenance wiring, `LLAMA_ARG_*` env note; P0 is first model-load smoke on `b11149`.
- [`2026-09-24-upstream-b11149-upgrade.md`](./2026-09-24-upstream-b11149-upgrade.md) — upstream runtime `b10867` → `b11149` (nightly CUDA 13.4, post-`v0.5.0`): `v0.5.0` tag ships no binaries (nightly-tag.txt only), dual-zip download/extract, `--version` 0.5.0-dev build 11149 with CUDA backend load, submodule to `v0.5.0`, User env repoint (Machine scope stale), `b10867` kept as rollback.
- [`2026-09-23-exl3-post-evaluation.md`](./2026-09-23-exl3-post-evaluation.md) — analytical pass on a viral EXL3 tier-by-tier recipe map: technology substrate (QTIP trellis + Hadamard, 1.6 bpw coherent Llama-3.1-70B) credited, performance claims not adopted as project guidance; maps post's tier cards to existing project docs (only the 8 GB rung has on-rig measurement — `fastest-tps-inference-engine.md` §3.1 EXL3 49.6 t/s vs llama.cpp 38.7/57.3, fair-MTP hardware-blocked); no fresh probe triggered; daily config unchanged.
- [`2026-09-17-qwen3.8-35b-a3b-trial.md`](./2026-09-17-qwen3.8-35b-a3b-trial.md) — Qwen3.8-35B-A3B Q4_K_M validation and full Trial @65536 q4_0: OOM / RAM Circuit Breaker diagnosis (mmap shareable working set vs Task Manager private working set, MoE lazy loading bursts), layer offload tuning (`N_CPU_MOE=31`, 10 GPU layers); full Trial on_front: TPS 42.8, bench_tg 33.5, VRAM 7.7GB, coding 0.4900, agentic_full 0.7333 (11/15).
- [`2026-09-16-k2-tps-oom.md`](./2026-09-16-k2-tps-oom.md) — K2-Horizon-7B and MoVA-36B-A4B TPS / small-ctx OOM diagnosis: untied 250k vocab (9.0B physical params), 100% full dense MHA KV footprint (40.5 KiB/tok), and MoVA multi-expert offload gap.
