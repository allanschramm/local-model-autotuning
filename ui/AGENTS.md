...
Tests: `tests/test_ui_server.py` — static-route security tests (real `HTTPServer` thread, ephemeral port). Static routes serve only exact allowlisted names (`_STATIC_FILES`: CSS + bundled OFL fonts); anything else → 404. Content-Type comes from the fixed table, never request data. Display-only HTTP smoke is E2E territory (root Testing rules).
## Child DOX Index
None — `ui/` is a leaf.
