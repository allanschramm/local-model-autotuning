...
- `rank_results.py` to print Day / Night / claw / coding / agentic-coding rankings from the results store — canonical `results.db` (SQLite) first, legacy `results.tsv` fallback. ADR 0017: the rank is a leaderboard of every complete model (quality-first, ±0.05 near-tie band; Day tie-break TPS, Night tie-break ctx); each row = the model's single best Trial; the `pareto_front` alias passes complete points through. Agents must use this CLI for model rankings — no ad-hoc temp filter scripts.
