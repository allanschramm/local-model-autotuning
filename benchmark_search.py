...
def run_benchmark(client: LlamaClient, **kwargs) -> BenchmarkResult:
    """Unified entry point for in-process orchestration."""
    ...
    return harness.evaluate(
        ...
        presence_penalty=kwargs.get("presence_penalty", PRES_PENALTY),
        frequency_penalty=kwargs.get("frequency_penalty", FREQ_PENALTY),
        repeat_penalty=kwargs.get("repeat_penalty", REP_PENALTY),
...
def main(model_name: str = MODEL_NAME, ctx_size: int | None = None, flash_attn: str | None = None, kv_cache: str | None = None) -> int:
    ...
    try:
        with LlamaServerRunner(intent) as runner:
            print("Server started. Running unified evaluation...")
...
def run_all_models():
    ...
    print(f"\n{'='*110}")
    print("BENCHMARK RESULTS (Unified Harness)")
    print(f"{'='*110}")
